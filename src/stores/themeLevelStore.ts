...
export const useThemeLevelStore = defineStore("themeLevel", () => {
  ...
  async function setCurrentTheme(shortName?: string): Promise<boolean> {
    ...
    mainConfigForCurrentTheme.value = mainConfig?.categories?.find(
      (cat: any) => cat.query_str === shortName,
    );
    //Get data from google sheets and store along with all the indicators in categoryConfigs
    const currentIndicatorConfigs: IndicatorConfig[] | null =
      getAllCurrentThemeIndicators();
  ...
  function getAllCurrentThemeIndicators(): IndicatorConfig[] | null {
    ...
    if (shortName) {
      ...
      const indicators =
        (categoryConfig?.indicators as IndicatorConfig[]) || [];
      ...
      return indicators.map((indicator: IndicatorConfig) => {
        ...
        if ((indicator as any)["style-exception"]) {
          indicator.style = (indicator as any)["style-exception"];
        } else {
          if (categoryStyle) {
